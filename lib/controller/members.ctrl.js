...
module.exports = {
    ...
    {
        connection.acquire(function (err, con) {
            con.query("SELECT *,(CONCAT(member_master.roomno,', ',member_master.building,',  ',member_master.street,', ',member_master.locality,'.')) AS location FROM `member_master` WHERE `nearme` != 1 AND `status` = 0 ORDER BY id DESC", function (err, result) {
                if(err)
                {
	...
	ListMembersNearMe: function(req,res)
    {
        connection.acquire(function (err, con) {
            con.query("SELECT *,(CONCAT(member_master.roomno,', ',member_master.building,',  ',member_master.street,', ',member_master.locality,'.')) AS location FROM `member_master` WHERE `nearme` = 1 AND `status` = 0 ORDER BY id DESC", function (err, result) {
                if(err)
    ...
    {
        connection.acquire(function (err, con) {
            con.query("SELECT (SELECT COUNT(*) FROM member_master WHERE member_master.nearme != 1 AND member_master.status = 0) as totalmembers, (SELECT COUNT(*) FROM member_master WHERE member_master.nearme = 1 AND status = 0) as contactnearme limit 1", function (err, result) {
                if(err)
                {
                    res.send({status:0,message:"Something went wrong"});
                }
